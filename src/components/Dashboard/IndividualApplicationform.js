...
function IndividualApplicationform() {
    ...
    const uploadAFS = (e) =>{
      ...
      formData.append('salesAgreementAppointmentDate', saad)
...
const uploadSAS = (e) =>{
  ...
  .then(response=>{
    ...
    const generateApfd = (e) =>{
      const Token = 'bearer' + " " + Cookies.get('Token')
      axios.post(`${BASE_URL}/api/v1/util/generateApplicationFormDraft`, {applicationId: applicationId, userFullName : Cookies.get('FullName')}, {headers:{'Authorization':Token}})
        .then(response=>{
            console.log(response)
            window.location.reload()
    ...
    useEffect(()=>{
      ...
      if(response.data.salesAgreementScan === false){
        ...
        for(var i=0;i<response.data.documents.length;i++){
          ...
          if(response.data.salesAgreementAppointmentDate){
          setSaads(response.data.salesAgreementAppointmentDate)
          }
            ...
            <Tab.Content>
                ...
                <button className="btn btn-danger" onClick={upload}>Upload Document</button> */}
                {
                  ...
                  <h4 style={{paddingRight:'10px', marginRight:'5px', fontSize:'22px', paddingTop:'5px', paddingLeft:'10px'}}>Application Form Draft </h4><br/>
                    ...
                    <h6><span style={{fontWeight:'bold', fontSize:'18px'}}>Generated By</span>: {duploadedby}</h6>
                ...
                
                {
                  ...
                  <div style={{display: 'flex'}}> 
                    ...
                    <button className="btn btn-secondary btn-user" type="submit">Upload Document</button>
                  </div>
                  <div className="row justify-content-center">
                    <div className="col-4">
                    <label>Sales Agreement Appointment Date</label><input className="form-control" type="date" min={tomorrow} required onChange={(e)=>setSaad(e.target.value)} />
                    </div>
                  </div>
                  </form>
                  </>:
                  ...
                  <h4 style={{paddingRight:'10px', marginRight:'5px', fontSize:'22px', paddingTop:'5px', paddingLeft:'10px'}}>Application Form Scan Copy </h4><br/>
                    ...
                    <h6><span style={{fontWeight:'bold', fontSize:'18px'}}>Uploaded Date: </span>{afsuploadeddate.split(' ')[0] +' '+afsuploadeddate.split(' ')[1]+' '+afsuploadeddate.split(' ')[2]+', '+afsuploadeddate.split(' ')[3]}</h6>
                ...
                
                {
                  ...
                  { saads === "" ?
                  <>
                  <h4>Sales Agreement Draft not generated. Agreement Date is not set.</h4>
                  </>
                  : 
                  <>
